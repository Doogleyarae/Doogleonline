...
- June 16, 2025. Removed sender account and transfer method fields from order form per user request - cleaned up exchange form by removing "Sender's Phone Number or Account Number" input field and "Select Transfer Method" dropdown. Updated database schema to remove sender_account and sender_method columns, simplified form validation, and maintained clean user experience focused on essential transaction details only
- June 16, 2025. Added conditional account number field to exchange form - displays phone number input for mobile money methods (Zaad, Sahal, EVC Plus, eDahab) and account number input for Premier Bank. Field only appears when sending with these specific payment methods, with appropriate placeholders and validation. Updated database schema with senderAccountNumber column and enhanced form data memory to preserve account details
- June 16, 2025. Implemented fully responsive admin dashboard with mobile-first design - added secure login authentication with session management, comprehensive input validation with specific error messages, confirmation dialogs for all critical actions, mobile-friendly card layouts for orders and data, enhanced forms with proper validation and success notifications. Dashboard now works perfectly on phones, tablets, and laptops with mobile navigation hamburger menu, desktop tabbed interface, professional authentication system, real-time validation for exchange rates/balances/limits, and responsive tables that convert to cards on mobile devices
...
- June 16, 2025. Restructured admin dashboard page layout per user request - moved "Current Exchange Rates" to top position and "Update Exchange Rate" form to second position. This improves user workflow by showing existing rates first before allowing updates, reducing errors and providing better context for rate management decisions
