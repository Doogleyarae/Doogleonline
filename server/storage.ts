  ...
  async updateBalance(insertBalance: InsertBalance): Promise<Balance> {
    ...
    try {
      ...
      if (currencyUpper && insertBalance.amount) {
        ...
        console.log(`[updateBalance] WebSocket notification sent for ${currencyUpper}: $${insertBalance.amount}`);
      }
  ...
  async updateOrderStatusWithBalanceLogic(orderId: string, status: string): Promise<Order | undefined> {
    ...
    try {
      ...
      if (status === "cancelled") {
        ...
        // === NEW LOGIC: Credit sender's balance when order is completed ===
        ...
        await this.createTransaction({
          ...
          description: `Order completed - ${sendAmountNum} ${sendCurrency} credited to sender balance`
        });
        ...
        // Remove sender credit logic from here. Only update order status.
        await db
          .update(orders)
          .set({ 
            status: status, 
